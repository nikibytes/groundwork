...
def extract_requirements(prd: str) -> list[dict]:
    ...
    for line_no, line in enumerate(prd.splitlines(), 1):
        ...
        if heading:
            ...
            current = {"id": f"PRD-{len(requirements) + 1:03d}", "title": title, "text": title, "source_line": line_no}
    ...
    if not requirements:
        for line_no, line in enumerate(prd.splitlines(), 1):
            ...
            if match:
                ...
                requirements.append({"id": f"PRD-{len(requirements) + 1:03d}", "title": text, "text": text, "source_line": line_no})
    return requirements
...
def rank_files(requirement: dict, files: list[dict], limit: int = 8) -> list[dict]:
    ...
    for item in files:
        ...
        if score:
            ranked.append({"path": path, "score": score, "language": item.get("language"), "purpose": item.get("purpose")})
...
def build_feature_map(root: Path, repo_map: dict) -> dict:
    ...
    if not prd_path.is_file():
        return {"schema_version": "0.1", "source": None, "requirements": [], "unavailable_reason": "docs/prd.md not found"}
    prd = prd_path.read_text(encoding="utf-8", errors="ignore")
    files = repo_map.get("files", [])
    mapped = []
    for requirement in extract_requirements(prd):
        candidates = rank_files(requirement, files)
        is_test = lambda p: bool(re.search(r"(^|/)(test|tests|spec|specs)(/|$)|(_test|\.test|\.spec)", p, re.I))
        implementation = [x for x in candidates if not is_test(x["path"])]
        tests = [x for x in candidates if is_test(x["path"])]
        mapped.append({"id": requirement["id"], "title": requirement["title"], "source": {"path": "docs/prd.md", "line": requirement["source_line"]}, "implementation_files": implementation, "test_files": tests, "status": "mapped" if implementation else "unmapped"})
    return {"schema_version": "0.1", "source": "docs/prd.md", "requirements": mapped}
