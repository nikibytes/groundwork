...
This is intentionally dependency-free. It observes repository facts without asking
an LLM to invent descriptions or changing source files.
"""
...
IGNORE_DIRS = {
    ".git", ".groundwork", "node_modules", "vendor", "dist", "build",
    ".next", ".nuxt", "coverage", "target", "__pycache__", ".venv", "venv",
}
...
def purpose_for(path: str) -> str:
    ...
    if name.lower().startswith("readme"):
        return "Project documentation and usage guide"
    if name.lower() in {"license", "license.md", "license.txt"}:
    ...
    if "/scripts/" in f"/{lower}/" or lower.startswith("scripts/"):
        return "Project automation script"
...
def package_managers(files: Iterable[str]) -> list[str]:
    ...
    if "pyproject.toml" in names or "requirements.txt" in names or "Pipfile" in names: managers.append("python")
...
def resolve_import(root: Path, source: str, target: str, language: str) -> str | None:
    if language in {"JavaScript", "TypeScript"} and target.startswith("."):
        ...
        candidates.extend(f"{base}/index{ext}" for ext in (".ts", ".tsx", ".js", ".jsx"))
        ...
        return None
    normalized = root.joinpath(candidates[0]).resolve() if candidates else None
...
def build_dependency_graph(root: Path, files: list[str]) -> dict:
    ...
    for source in files:
        ...
        for pattern in IMPORT_PATTERNS[language]:
            for match in pattern.finditer(text):
                ...
                if not target:
                    continue
                resolved = resolve_import(root, source, target, language)
    ...
    return {
        "nodes": [{"path": f, "language": language_for(f)} for f in files],
        "edges": sorted(unique.values(), key=lambda e: (e["from"], e["to"])),
    }
...
def analyze(root: Path) -> dict[str, dict]:
    ...
    try:
        ...
        git = {"is_repository": True, "head": commit, "branch": branch}
    except (subprocess.CalledProcessError, FileNotFoundError):
    ...
    repo_map = {
        ...
        "files": [
            {"path": f, "purpose": purpose_for(f), "language": language_for(f)}
            for f in files
        ],
    }
    graph = build_dependency_graph(root, files)
    return {"project": project, "repo_map": repo_map, "dependency_graph": graph}
